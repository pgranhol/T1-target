...
{ 
  ...
  if( command == fThickCmd )
   { fDetector->SetTargetLength(fThickCmd->GetNewDoubleValue(newValue));}

  //  if( command == fSizeYZCmd )
    //   { fDetector->SetTargetRadius(fSizeYZCmd->GetNewDoubleValue(newValue));}
  ...
  if (command == fIsotopeCmd)
   {
     ...
     fDetector->SetTargetMaterial(name);    
  }   
}
